...
description: 'Transport solution components from a preview environment to dev. Use when: promoting a feature from preview to integration, completing inner-loop development, handing off a validated feature, moving components from a feature solution to the main solution.'
---

# Transport Solution (Move to Dev)

Move a validated feature from preview/preview-test into the dev integration environment and the `develop` branch. This is the final inner-loop step before a release.

## When to Use

- A feature has been developed and tested in preview-test and is ready for integration
- Components need to move from a feature solution to the main solution in the dev environment
...
> Before proceeding, read `deployments/settings/environment-config.json`. Use `solutionAreas[x].previewEnv` and `solutionAreas[x].devEnv` slugs, and resolve URLs from `innerLoopEnvironments[].url`. Do not use hardcoded values.
...
Read from `environment-config.json`: for each solution area, `solutionAreas[x].previewEnv` is the source slug and `solutionAreas[x].devEnv` is the target slug. Resolve both URLs from `innerLoopEnvironments[].url`.
...
- `source_environment_url`: Preview environment URL (from `innerLoopEnvironments[]` in environment-config.json)
- `target_environment_url`: Dev environment URL (from `innerLoopEnvironments[]` in environment-config.json)
...
- The main solution in the dev environment contains the transported components
...
- The feature solution in the preview environment can be deleted from Dataverse (optional but recommended to keep environment tidy)
