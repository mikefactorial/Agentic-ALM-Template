...
12. **Dataverse Display Names must be Title Case with spaces** — when creating tables, columns, choices, or any Dataverse component, Display Names must use Title Case with spaces between words (`Suggested Priority`, `Signal Transcript`). Schema names use PascalCase with the publisher prefix (`{prefix}_SuggestedPriority`); logical names are all lowercase (`{prefix}_suggestedpriority`). Never concatenate words without spaces in a Display Name.
