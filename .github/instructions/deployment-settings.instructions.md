...
11. **`managedIdentities[].name` must be the `managedidentityid` GUID** — Power Platform does not always write a `<name>` child element to `customizations.xml`. The only reliable lookup key is the `managedidentityid` attribute (a GUID). Find it in `src/solutions/{solution}/ManagedIdentities/{folder}/managedidentity.xml` or via `pac managed-identity get --environment <url>`. Never use a display name string — it will silently fail to match.
